...
# Use DATABASE_URL if available (for CI/CD and simple configuration)
...
# Update Celery result backend based on database configuration
if os.getenv("DATABASE_URL"):
    # For SQLite, use django-db backend
    if "sqlite" in os.getenv("DATABASE_URL", ""):
        CELERY_RESULT_BACKEND = "django-db"
    else:
        # For PostgreSQL, construct the connection string
        parsed = urlparse(os.getenv("DATABASE_URL"))
        CELERY_RESULT_BACKEND = f"db+{parsed.scheme}://{parsed.netloc}{parsed.path}"
else:
